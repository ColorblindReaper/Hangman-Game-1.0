...
def play(word):   #тело функции игры
    tries = 6 # количество попыток
    guessed_words = [] # список уже названных слов
    guessed_letters = [] # список уже названных букв
    word_completion = '_' * len(word) # строка, содержащая символы _ на каждую букву задуманного слова
    symbols = ('!@№$;%^:&?*\)(<>|/-+=_~`[]{},.') # символы для игнорирования в случае ввода
    print('Сыграем в виселицу!', 'Lets play Hangman Game')
    print(word_completion) # загаданное слово
    display_hangman(tries) # отображение рисунка виселицы
    guessed = False # сигнальная метка


    while tries != 0 and guessed == False:
        ask = input().upper() # вводимое слово или буква возводим в верхний регистр
        while ask.isalpha() == False or ask in symbols or ask in guessed_words:    # делаем цикл пока введённая буква или слово не подходит под требуемое запрашиваем ввод
            ask = input('Введите другую букву или слово. Enter another letter or word:').upper()
            print()
        if ask in word and len(ask) == 1:  # если введённая строка = длине 1 символа значит это буква и если она есть в слове запускаем цикл
            guessed_letters.append(ask)
            for i in range(len(word)):
                if word[i] == ask:
                    word_completion = word_completion[:i] + ask + word_completion[i+1:]
            print(word_completion)
            if word_completion == word:
                guessed = True
                return print('Поздравляем, вы отгадали слово. Вы выиграли! Congratulations, you are guessing word. You win!')
        elif ask == word: # если введённое слово = загадонному = цикл
            guessed = True
            return print('Поздравляем, вы угадали слово. Вы выиграли! Congratulations, you are guessing word. You win!')
        elif ask != word and len(ask) == len(word): # если введённое слово равно длине загадонного но не равно ответу = цикл
            tries -= 1
            print(display_hangman(tries))
            guessed_words.append(ask)
        elif ask not in word and len(ask) == 1: # если буква не в слове = цикл
            guessed_letters.append(ask)
            tries -= 1
            print(display_hangman(tries))
    return print(f'Вы проиграли, загаданное слово было {word} \nYou lose the hidden word was {word}') # возврат функции в случае его завершения по условию

play(word) # запуск функции игры
